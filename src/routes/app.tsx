      ...
      const dias = fields.dias_afastamento
        ...
        : fields.data_inicio && fields.data_fim
          ? Math.max(
              1,
              Math.round(
                (new Date(fields.data_fim).getTime() - new Date(fields.data_inicio).getTime()) /
                  86400000,
              ) + 1,
            )
          : null;

      const { data: inserted, error: insErr } = await supabase.from("atestados").insert({
        user_id: user.id,
      ...
      <section className="md:col-span-5 space-y-6 animate-in-up">
        ...
        {!file ? (
          ...
          <div className="border-2 border-border rounded-2xl p-3 bg-card">
            {preview ? (
              <img src={preview} alt="Pré-visualização do atestado" className="w-full aspect-[3/4] object-cover rounded-xl bg-muted" />
            ) : (
              <div className="w-full aspect-[3/4] rounded-xl bg-muted grid place-items-center">
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-2 font-mono">{file.name}</p>
                </div>
              </div>
            )}
      ...
      <section className="md:col-span-7 space-y-6 animate-in-up">
        {busy === "ocr" && (
          <div className="bg-card border border-border rounded-2xl p-8 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1 font-mono uppercase tracking-wider">Aguarde um instante</p>
          </div>
        )}
        ...
        {fields && (
          <div className="bg-card border border-border rounded-2xl p-5 md:p-6 space-y-5 shadow-sm">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-xs uppercase tracking-widest text-muted-foreground">Dados do atestado · revise</h3>
              {confianca !== null && (
            ...
            <div className="grid gap-4">
              <Field label="Nome do paciente" value={fields.paciente_nome} onChange={(v) => setFields({ ...fields, paciente_nome: v })} />
              <div className="grid grid-cols-2 gap-4">
                <Field label="CPF" value={fields.cpf} onChange={(v) => setFields({ ...fields, cpf: v })} />
                <Field label="Matrícula" value={fields.matricula} onChange={(v) => setFields({ ...fields, matricula: v })} />
              </div>
              <div className="grid grid-cols-3 gap-4">
                <Field label="Emissão" type="date" value={fields.data_emissao} onChange={(v) => setFields({ ...fields, data_emissao: v })} />
                <Field label="Início" type="date" value={fields.data_inicio} onChange={(v) => setFields({ ...fields, data_inicio: v })} />
                <Field label="Fim" type="date" value={fields.data_fim} onChange={(v) => setFields({ ...fields, data_fim: v })} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Médico" value={fields.medico_nome} onChange={(v) => setFields({ ...fields, medico_nome: v })} />
                <Field label="CRM" value={fields.medico_crm} onChange={(v) => setFields({ ...fields, medico_crm: v })} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Field label="CID" mono value={fields.cid} onChange={(v) => setFields({ ...fields, cid: v })} />
                <div className="space-y-1.5">
                  <Label className="text-[11px] font-bold uppercase text-muted-foreground">Dias de afastamento</Label>
                  <Input
                    type="number"
                    value={fields.dias_afastamento ?? ""}
                    onChange={(e) => setFields({ ...fields, dias_afastamento: e.target.value ? Number(e.target.value) : undefined })}
                  />
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  onChange={(e) => setFields({ ...fields, observacoes: e.target.value })}
                />
              </div>
            </div>
            <Button onClick={submit} disabled={busy === "saving"} className="w-full font-semibold py-6">
              {busy === "saving" ? <Loader2 className="size-4 mr-2 animate-spin" /> : <Check className="size-4 mr-2" />}
        ...
        <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
          <div className="px-5 md:px-6 py-4 border-b border-border bg-muted/30">
            <h2 className="font-bold text-xs uppercase tracking-widest">Histórico</h2>
          </div>
          <div className="divide-y divide-border">
            {(historico ?? []).length === 0 && (
              <p className="p-6 text-sm text-muted-foreground text-center">Nenhum atestado enviado ainda.</p>
            )}
            {historico?.map((a) => (
              <div key={a.id} className="px-5 md:px-6 py-4 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold truncate">{a.paciente_nome || "(Sem nome)"}</p>
                  <p className="text-[11px] font-mono text-muted-foreground mt-0.5">
                    {a.cid ? `CID: ${a.cid} · ` : ""}{a.dias_afastamento ?? "—"} dias · {new Date(a.created_at).toLocaleDateString("pt-BR")}
                  </p>
                </div>
