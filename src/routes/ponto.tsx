...
import { Clock, LogIn, Coffee, LogOut as LogOutIcon, ArrowRight } from "lucide-react";
...
function Page() {
  ...
  useEffect(() => { if (!loading && !session) nav({ to: "/login" }); }, [loading, session, nav]);
...
function Content() {
  ...
  const { data: profile } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("profiles").select("tipo_ponto").eq("id", user!.id).maybeSingle();
      return data;
    },
    enabled: !!user,
  });
  ...
  const { data: todayEntries } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("time_entries")
        ...
        .gte("entry_at", startOfDay).lte("entry_at", endOfDay)
        .order("entry_at");
      return data ?? [];
    },
    enabled: !!user,
  ...
  const bater = async () => {
    if (!nextTipo) { toast.info("Já bateu todos os pontos do dia."); return; }
    const { error } = await supabase.from("time_entries").insert({
      user_id: user!.id, tipo: nextTipo, origem: "app",
    });
  ...
  return (
    <section className="space-y-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight">Ponto eletrônico</h1>
      ...
      <div className="rounded-xl border border-border p-6 text-center space-y-4">
        <div className="text-5xl font-mono font-bold tabular-nums">
          {Math.floor(workedMin / 60).toString().padStart(2, "0")}:{(workedMin % 60).toString().padStart(2, "0")}
        </div>
      ...
      <div className="space-y-2">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Batidas de hoje</h2>
        {!todayEntries?.length && <div className="text-sm text-muted-foreground">Nenhuma batida ainda.</div>}
            ...
            return (
              <div key={e.id} className="flex items-center justify-between rounded-md border border-border p-3 text-sm">
                ...
                <span className="font-mono text-muted-foreground">
                  {new Date(e.entry_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
                </span>
              </div>
            );
          })}
