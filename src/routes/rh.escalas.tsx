...
function Page() {
  ...
  useEffect(() => {
    ...
    else if (!hasPermission("configure_schedules")) {
...
function Content() {
  ...
  const { data: schedules } = useQuery({
    queryKey: ["schedules"],
    queryFn: async () =>
      (await supabase.from("work_schedules").select("*").order("nome")).data ??
      [],
  });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Defina jornadas e atribua a funcionários.
            </p>
          </div>
        </div>
          ...
          >
            <div className="space-y-1.5">
              <div className="font-semibold flex items-center gap-2">
                {s.nome}
                <Badge
                  variant={s.tipo === "fixed_weekly" ? "secondary" : "outline"}
                >
...
function ScheduleDialog({ onSaved }: { onSaved: () => void }) {
  ...
  const [weekly, setWeekly] = useState<
    Record<
      string,
      { start: string; end: string; break_min: number; on: boolean }
    >
  >(
    Object.fromEntries(
      DAYS.map((d) => [
        d.key,
        {
          ...
          break_min: 60,
          on: ["mon", "tue", "wed", "thu", "fri"].includes(d.key),
        },
      ]),
    ),
  );
  ...
  const save = async () => {
    ...
    if (tipo === "fixed_weekly") {
      config = Object.fromEntries(
        Object.entries(weekly)
          ...
          .map(([k, v]) => [
            k,
            { start: v.start, end: v.end, break_min: v.break_min },
          ]),
      );
    } else {
      config = {
    ...
    const { error } = await supabase.from("work_schedules").insert({
      ...
      carga_horaria_mensal: Number(carga) || 220,
    });
    setSaving(false);
    if (error) return toast.error(error.message);
      ...
      <div className="space-y-4">
        <div className="space-y-1.5">
          ...
          <Input
            ...
            placeholder="Ex: Comercial 8h-17h"
          />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <Select
              value={tipo}
              onValueChange={(v) => setTipo(v as "fixed_weekly" | "rotative")}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
        ...
        {tipo === "fixed_weekly" ? (
            ...
            {DAYS.map((d) => (
              <div key={d.key} className="flex items-center gap-2">
                <label className="flex items-center gap-1.5 w-16 text-sm">
                  <input
                    type="checkbox"
                    checked={weekly[d.key].on}
                    onChange={(e) =>
                      setWeekly({
                        ...weekly,
                        [d.key]: { ...weekly[d.key], on: e.target.checked },
                      })
                    }
                  />
                  {d.label}
                ...
                <Input
                  type="time"
                  value={weekly[d.key].start}
                  disabled={!weekly[d.key].on}
                  onChange={(e) =>
                    setWeekly({
                      ...weekly,
                      [d.key]: { ...weekly[d.key], start: e.target.value },
                    })
                  }
                  className="flex-1"
                />
                <Input
                  type="time"
                  value={weekly[d.key].end}
                  disabled={!weekly[d.key].on}
                  onChange={(e) =>
                    setWeekly({
                      ...weekly,
                      [d.key]: { ...weekly[d.key], end: e.target.value },
                    })
                  }
                  className="flex-1"
                />
                <Input
                  ...
                  onChange={(e) =>
                    setWeekly({
                      ...
                      [d.key]: {
                        ...weekly[d.key],
                        break_min: Number(e.target.value),
                      },
                    })
                  }
                  className="w-20"
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <Select
                value={pattern}
                onValueChange={(v) => setPattern(v as "12x36" | "6x1" | "5x2")}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
