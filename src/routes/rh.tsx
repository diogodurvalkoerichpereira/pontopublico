...
function RhPage() {
  ...
  return <AppShell>{isRhHome ? <RhContent /> : <Outlet />}</AppShell>;
}
    ...
    <div className="space-y-8 animate-in-up">
      <div>
        ...
        <p className="text-muted-foreground mt-2 text-sm">
          Aprovação e auditoria de atestados médicos.
        </p>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
        <div className="px-5 md:px-6 py-4 border-b border-border bg-muted/30 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 text-xs">
            ...
            {(["pendente", "aprovado", "rejeitado", "todos"] as const).map(
              (s) => (
                <button
                  key={s}
                  onClick={() => setFilter(s)}
        ...
        <div className="divide-y divide-border">
          ...
          {list?.map((a) => (
            <div
              key={a.id}
              className="px-5 md:px-6 py-4 flex flex-col md:flex-row md:items-center gap-3 md:gap-4"
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] font-mono text-muted-foreground mt-0.5 truncate">
                    {a.cid && `CID ${a.cid} · `}
                    {a.dias_afastamento ?? "—"}d · {a.data_inicio || "?"} →{" "}
                    {a.data_fim || "?"} · Dr(a) {a.medico_nome || "—"}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => download(a.arquivo_path)}
...
function Stat({
  ...
  return (
    <div className="bg-card border border-border p-4 md:p-5 rounded-2xl">
      ...
      <p className={`text-2xl md:text-3xl font-extrabold mt-1 ${accent}`}>
        {value ?? "—"}
      </p>
    </div>
  );
