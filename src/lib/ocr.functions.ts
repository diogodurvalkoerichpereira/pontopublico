...
const OCR_TOOL = {
  ...
  function: {
    ...
    description: "Extrai os campos estruturados de um atestado médico brasileiro.",
    parameters: {
      type: "object",
      properties: {
        paciente_nome: { type: "string", description: "Nome completo do paciente" },
        cpf: { type: "string", description: "CPF do paciente, apenas dígitos ou formato ###.###.###-##" },
        matricula: { type: "string", description: "Matrícula do funcionário, se constar" },
        data_emissao: { type: "string", description: "Data de emissão no formato YYYY-MM-DD" },
        data_inicio: { type: "string", description: "Data inicial do afastamento YYYY-MM-DD" },
        data_fim: { type: "string", description: "Data final do afastamento YYYY-MM-DD" },
        dias_afastamento: { type: "number", description: "Quantidade de dias de afastamento" },
        medico_nome: { type: "string", description: "Nome do médico" },
        medico_crm: { type: "string", description: "CRM do médico, com UF se houver" },
        cid: { type: "string", description: "Código CID-10 (ex: J06.9). Vazio se não constar." },
        observacoes: { type: "string", description: "Outras observações relevantes" },
  ...
  .handler(async ({ data }) => {
    ...
    const userContent = isPdf
      ? [
          ...
          {
            type: "file",
            file: { filename: "atestado.pdf", file_data: `data:application/pdf;base64,${data.base64}` },
          },
        ]
      : [
          {
    ...
    try {
      const res = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          tools: [OCR_TOOL],
          tool_choice: { type: "function", function: { name: "extract_atestado" } },
        }),
      });

      if (res.status === 429) return { ok: false as const, error: "Limite de uso atingido. Tente novamente em alguns instantes." };
      ...
      if (!call?.function?.arguments) {
        return { ok: false as const, error: "Não foi possível extrair os campos." };
      }
