...
| **Campo com lista fechada não se digita.** Use `<SelectClassificacao>` (catálogo) ou um `<Select>` dos registros.                                                                                  | Classificação digitada à mão produz código inexistente e o mesmo código em grafias divergentes ("3.1.90.11" e "319011"), que o agrupamento soma em linhas separadas. O catálogo vive em `budget_reference_codes` (padrão nacional + os do ente) e aceita código fora dele, porque o TCE do ente pode exigir o seu. Rede: `tests/campos-com-opcoes.test.mjs`. |
