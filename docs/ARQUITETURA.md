...
| Ponto                    | `time_clock_punches` (marcação imutável encadeada, NSR por ente) + `time_clock_counters` (O1-03a); espelho/comprovante em `time-mirror.ts` (O1-03c); `time_entries`/`work_schedules` (geração legada, user-scoped) |
